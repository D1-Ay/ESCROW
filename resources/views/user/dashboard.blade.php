...
@section('content') 
    ...
    @php
        $usdWalletShown = false;  // Initialize flag before the loop
    @endphp

    @foreach ($userWallet as $item)
        <!-- Check if the current wallet is USD and if the flag is still false -->
        @if ($item->currency->code === 'USD' && !$usdWalletShown)
            ...
            <!-- Set the flag to true after rendering the first USD wallet -->
            @php
                $usdWalletShown = true;
            @endphp
        @endif
